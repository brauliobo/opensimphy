...
function parseTap(
  ...
  const step = tap.step === undefined ? 0 : requireSafeIntegerBetween(tap.step, `${path}.step`, 0, steps)
...
function parseInput(value: unknown, descriptor: AwesomePhysicsSimulationDescriptorV1): ParsedInput {
  ...
  const gridSize = requireSafeIntegerBetween(input.gridSize, 'input.gridSize', 3, maxGridSize)
