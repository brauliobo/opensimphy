...
const selected = ref<{ row: number; column: number; value: string | number | null; isExactZero: boolean | null } | null>(null)
...
function color(value: string | number | null): string {
  ...
  if (props.result.mode === 'mod') {
    const minimum = props.result.min ?? 0
    const maximum = props.result.max ?? 0
    const range = Math.max(1, maximum - minimum)
    const ratio = (numberValue - minimum) / range
    return `hsl(${Math.round(30 + ratio * 160)} 58% ${Math.round(30 + ratio * 32)}%)`
  }
  const range = Math.max(Math.abs(props.result.min ?? 0), Math.abs(props.result.max ?? 0), 1)
...
function selectCoordinates(row: number, column: number): void {
  const values = props.result.values[row] ?? []
  if (!values.length) return
  column = Math.min(values.length - 1, Math.max(0, column))
  selected.value = {
    row,
    column,
    value: values[column] ?? null,
    isExactZero: props.result.exactZeroMask[row]?.[column] ?? null,
  }
}
...
function selectCell(event: MouseEvent): void {
  ...
  const columns = props.result.values[row]?.length ?? 0
  const column = Math.min(columns - 1, Math.max(0, Math.floor((event.offsetX / bounds.width) * columns)))
  selectCoordinates(row, column)
}

function inspectWithKeyboard(event: KeyboardEvent): void {
  if (!['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) return
  event.preventDefault()
  const lastRow = Math.max(0, props.result.values.length - 1)
  let row = selected.value?.row ?? 0
  let column = selected.value?.column ?? 0
  if (event.key === 'ArrowUp') row = Math.max(0, row - 1)
  if (event.key === 'ArrowDown') row = Math.min(lastRow, row + 1)
  if (event.key === 'ArrowLeft') column -= 1
  if (event.key === 'ArrowRight') column += 1
  if (event.key === 'Home') column = 0
  if (event.key === 'End') column = Math.max(0, (props.result.values[row]?.length ?? 1) - 1)
  selectCoordinates(row, column)
}
...
.wall-figure
  .wall-canvas-wrap
    canvas(
      ...
      tabindex="0"
      role="img"
      :aria-label="`Number wall, ${result.depth} rows by ${result.width} columns; use arrow keys to inspect display cells`"
    ...
    p(:id="descriptionId") Focus the wall and use arrow keys to move by cell. Home and End move to the first and last column.
  .wall-legend
    span {{ result.min ?? 'no finite display value' }}
    i(aria-hidden="true")
    span {{ result.max ?? 'no finite display value' }}
    span Exact zero cells: {{ result.zeroCount }}
  output.cell-readout(:id="readoutId" aria-live="polite")
    template(v-if="selected") Cell [{{ selected.row }}, {{ selected.column }}] = {{ selected.value }}; {{ selected.isExactZero === null ? 'outside the computed domain' : selected.isExactZero ? 'exact zero' : 'not an exact zero' }}.
    template(v-else) Select a cell with a pointer or keyboard to inspect its display value.
</template>
