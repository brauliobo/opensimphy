...
| Gray production FEM LUT | Absent; 24 GiB GetDP cap cannot complete 33-sample production; fail-closed |
...
- FEM v1 is immutable rejected evidence: its partition-growth and outer-domain gates failed. The v2 pilot report passed its declared formulation/mesh/energy/current/periodicity criteria and records a 1.1584935659% coarse/fine observation, but its pass tolerance is not an uncertainty bound. Production v2 remains rejected at 18/33 required samples, so no production LUT is published. The 2026-08-16 recheck found the pinned solver image present, then failed closed before GetDP because publication still requires an approved 33-sample report; direct MUMPS stays capped at 24 GiB and the accepted 0.025 m coarse mesh.
