...
describe('EARTH method registry', () => {
  it('defines 136 methods with two per pilot, two on PRT-001 and NUC-004, and one per other program', () => {
    ...
    expect(SUPPORTED_EARTH_SIMULATION_IDS.flatMap((programId) => listEarthMethods(programId))).toHaveLength(136)
    ...
    for (const programId of SUPPORTED_EARTH_SIMULATION_IDS) {
      ...
      const extraCount = programId === 'EARTH-PRT-001' || programId === 'EARTH-NUC-004' ? 2 : undefined
  ...
  it('publishes clone-safe, JSON-safe, finite, runnable defaults for all 136 methods', () => {
    ...
    for (const programId of SUPPORTED_EARTH_SIMULATION_IDS) {
      for (const { id: methodId } of listEarthMethods(programId)) {
        ...
        } else if (programId === 'EARTH-FLD-006') {
          expect(result.predictions.map(({ claimId }) => claimId), `${programId}/${methodId}`).toEqual(['FLD-006-TC'])
        } else if (programId === 'EARTH-FLD-007') {
          expect(result.predictions.map(({ claimId }) => claimId), `${programId}/${methodId}`).toEqual(['FLD-007-FLOQ'])
        } else if (programId === 'EARTH-FLD-008') {
          expect(result.predictions.map(({ claimId }) => claimId), `${programId}/${methodId}`).toEqual([
            'FLD-008-SG', 'FLD-008-E', 'FLD-008-W',
          ])
        ...
        } else if (programId === 'EARTH-NUC-004' && methodId === 'coupling-force-hierarchy') {
          expect(result.predictions.map(({ claimId }) => claimId), `${programId}/${methodId}`).toEqual([
  ...
  it('uses explicit truthful provenance combinations', () => {
    ...
    expect(reproductions).toHaveLength(39)
