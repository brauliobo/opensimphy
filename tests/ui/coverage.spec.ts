...
describe('CoverageStrip', () => {
  it('fails closed when any exact count mismatches', () => {
    const wrapper = mount(CoverageStrip, { props: { rows: snapshot(false).coverage, complete: false } })
  ...
  it('only reports complete when explicitly supplied as exact', () => {
    const wrapper = mount(CoverageStrip, { props: { rows: snapshot(true).coverage, complete: true } })
