...
test.describe('Edwin Gray Workbench', () => {
  ...
  test('mounts persisted 100-rev COP, schematic cues, and whisper artifacts without a production LUT', async ({ page }) => {
    ...
    await expect(page.getByTestId('gray-cop-catalog')).toContainText('ema4')
    ...
    await expect(page.getByTestId('gray-claims-clip-cues')).toContainText('hidden secret number four')
  })
})
