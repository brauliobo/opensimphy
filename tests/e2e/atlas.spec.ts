...
test('discovers the EARTH dossier and browses its locked corpus without executing source content', async ({ page }) => {
  ...
  await expect(page.getByTestId('earth-evidence-ledger')).toContainText('222')
  await expect(page.getByTestId('earth-evidence-ledger')).toContainText('136 runnable')
...
test('filters and pages the URL-backed EARTH program registry, then restores state on back', async ({ page }) => {
  ...
  await expect(page.getByTestId('program-registry-statement')).toContainText('222 declared methods')
  await expect(page.getByTestId('program-registry-statement')).toContainText('136 runnable: 39 source reproductions / 97 traditional comparisons or contract validators; 86 unavailable source formulations')
