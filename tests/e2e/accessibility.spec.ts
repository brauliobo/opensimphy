...
test('manages mobile navigation and route heading focus without stealing initial focus', async ({ page }) => {
  ...
  await expect(page.locator('.app-footer')).toHaveAttribute('inert', '')
  await expect(page.locator('.app-footer')).toHaveAttribute('aria-hidden', 'true')
  ...
  await expect(page.locator('#primary-navigation').getByRole('link', { name: /Evidence/ })).toHaveAttribute('aria-current', 'location')
  await expect(page.locator('#primary-navigation').getByRole('group', { name: 'Reading depth' })).toBeVisible()
  await expect(page.locator('#primary-navigation').getByTestId('reading-depth-guided')).toBeChecked()

  await page.locator('.skip-link').focus()
  await expect(page.locator('#primary-navigation a').first()).toBeFocused()
  const technicalDepth = page.getByTestId('reading-depth-technical')
  await technicalDepth.check()
  await technicalDepth.focus()
  await page.keyboard.press('Tab')
  await expect(toggle).toBeFocused()
  await page.keyboard.press('Shift+Tab')
  await expect(technicalDepth).toBeFocused()
  ...
  const programsLink = page.locator('.earth-local-nav a[href="/earth/programs"]')
  await toggle.click()
  await expect(toggle).toHaveAttribute('aria-expanded', 'true')
  await programsLink.evaluate((link) => {
...
test('keeps formula audits, keyboard controls, and meaning-first detail operable at 320px and 400%-equivalent reflow', async ({ page }) => {
  ...
  await expect(page.locator('.formula-audit').first()).toBeVisible()
  await expect(page.locator('.formula-audit').first()).toContainText('source digit criterion')
  await expect(page.locator('.formula-audit').first()).toContainText('dimension audit')
  await expectNoDocumentOverflow(page)

  const filters = page.getByTestId('advanced-filters')
  const filterSummary = filters.locator('summary')
  await filterSummary.focus()
  await page.keyboard.press('Enter')
  await expect(filters).toHaveAttribute('open', '')
  await expectTargetSize(filterSummary)
  await expectTargetSize(page.getByTestId('formula-source-criterion'))
  await expectTargetSize(page.getByTestId('formula-dimension-audit'))
  await expectNoDocumentOverflow(page)

  await page.goto('/atlas/1')
  await expect(page.getByTestId('formula-record-ready')).toBeVisible()
  await expect(page.getByTestId('guided-meaning')).toBeVisible()
  await expect(page.getByTestId('meaning-caveats')).toBeVisible()
  await expect(page.getByTestId('sweep-finding')).toHaveAttribute('role', 'status')
  await expect(page.getByTestId('sweep-finding')).toHaveAttribute('aria-live', 'polite')
  await expectTargetSize(page.getByTestId('reveal-equation-step'))
  await expectTargetSize(page.getByTestId('save-formula-run'))
  await expectTargetSize(page.getByTestId('atlas-return'))

  const technicalDepth = page.getByTestId('formula-meaning').getByTestId('reading-depth-technical')
  await technicalDepth.check()
  await expect(page.getByTestId('source-dependency-trace')).toBeVisible()
  await expect(page.getByTestId('runtime-dependency-trace')).toBeVisible()
  await expect(page.getByTestId('dimension-basis-note')).toBeVisible()
  await expectNoDocumentOverflow(page)
})
