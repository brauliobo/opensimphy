...
The STEP worker performs a real meshStep conversion. The serial Gmsh worker independently opens the locked `cube.geo` built-in-kernel fixture and meshes it. OCC STEP is also a solver path: the workbench opens `cube.step` (or an uploaded STEP) through Gmsh OCC, assigns ground/electrode/volume physical groups from bounding boxes, and runs GetDP electrostatics. meshStep remains the fast preview; simulation-bound tags stay on the Gmsh surface. Preview selection handoff fails closed unless geometric area/centroid/normal signatures produce one complete unique face bijection, then stores the matched Gmsh entity key.
...
Deployment has two explicit profiles. GitHub Pages and the local production gate use `npm run build:deploy`; Pages wraps it with SPA fallback generation. The command verifies an already staged exact runtime and invokes Vite; it never compiles or generates a release. Compilation stays on the maintainer machine. GitHub fetches the locked `wasm-<contentVersion>` release, then runs the no-compile site build and ONELAB E2E. Staging, packaging, and publication are explicit local commands described in [WASM release distribution](wasm-releases.md). `VITE_BASE_PATH` remains inherited by the Vite build.

The `/labs/onelab` route is always registered and lazy-loaded. WASM assets still load only when that page mounts. Netlify runs the ordinary no-Docker `npm run build`. Solves still require the staged immutable locked asset tree; without it the workbench renders and warm fails closed. There is intentionally no invented download URL or committed binary fallback.
...
SLEPc is the default eigensolver in the locked GetDP WASM (`ENABLE_SLEPC=ON`, sequential, no MPI). The workbench cube cavity runs GetDP `EigenSolve` on the Gmsh mesh. Linking a newer SLEPc still requires a new content version and native reference gate.
